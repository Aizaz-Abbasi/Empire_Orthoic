...
struct EditorView: View {
    ...
    @State private var mode: DrawingMode = .text
    ...
    @State private var selectedTextView: UIView? = nil
    ...
    init(media: MediaItem, onClose: @escaping () -> Void, onSaveImage: @escaping (UIImage) -> Void) {
        self.media = media
...
//                            .onChange(of: font, perform: onFontChanged)
                ...
                GeometryReader { geometry in
                    ...
                    .background(
                        // ML canvas
                        CanvasView(canvas: $mlCanvas, shouldBecameFirstResponder: true)
...
//                        }.disabled(!canUndo)
    ...
    func saveImage() {
        print("saveImage")
            ...
            if media.type == .image {
                let image =  MediaProcessor.exportImage(image: media.image!, markup: markup, contentMode: contentMode, imageView: imageView)
                imageView.image = image
                withAnimation { isProcesing = false }
                onSaveImage(image ?? UIImage(named: "venice")!)
            }
    }
    ...
    func calculateImageSize(_ frame: CGRect) {
        ...
        if size.height >= size.width {
            ...
            if CGFloat(newWidth) > (frame.size.width) {
                ...
                newHeight = newHeight + CGFloat(diff) / newHeight * newHeight+300
...
struct EditorView_Previews: PreviewProvider {
    static var previews: some View {
        EditorView(media: MediaItem(type: .image, image: UIImage(named: "venice"), video: nil, videoUrl: nil), onClose: { }, onSaveImage: {_ in })
    }
}
