...
extern NSString* const kSTTrackerMinimumDepthForICPKey;

#pragma mark - STTracker Base
...
typedef NS_ENUM(NSInteger, STTrackerType) {
    ...
    */
    ...
    /** Specifies a tracker that will use both the depth information from Structure Sensor and the color information
     from the iOS device camera.
...
typedef NS_ENUM(NSInteger, STTrackerSceneType) {
    ...
    */
    ...
    Assumes that the sensor is moving and can rotate and translate. The scene captured should be on the scale of a
    typical room scan.
    */
...
typedef NS_ENUM(NSInteger, STTrackerQuality) {
    ...
    /// Designed for very fast tracking, it works best when tracking against a static mesh (e.g. after a scan has been
    /// done), or when the available CPU resources are limited.
...
typedef NS_ENUM(NSInteger, STTrackerPoseAccuracy) {
    /// The tracker cannot provide any accuracy information. This typically happens when it gets lost or before it sees
    /// any frame.
    STTrackerPoseAccuracyNotAvailable,

    /// The tracker pose accuracy is very low. This can happen when the model has been out of view for a long time or if
    /// very few pixels have depth data.
    STTrackerPoseAccuracyVeryLow,

    /// The tracker pose accuracy is low. This can happen when tracking against a model that is getting too close or out
    /// of view.
