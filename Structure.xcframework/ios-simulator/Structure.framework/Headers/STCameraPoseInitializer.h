...
typedef NS_ENUM(NSInteger, STCameraPoseInitializerStrategy) {
    ...
    If no ground plane is found or if the device is not looking downward, place the scanning volume at the distance
    given by the central depth pixels. In both cases, the cuboid orientation will be aligned with the provided gravity
    vector.
    ...
    */
    ...
    /** Align the camera orientation using the gravity vector, and places the camera center at the center of the
    scanning volume.
    __Note:__ This strategy does not require depth information.
    */
...
- Parameter volumeSize: The current volume size in meters. Not used by the
`STCameraPoseInitializerStrategyGravityAlignedAtOrigin` strategy.
...
- `kSTCameraPoseInitializerStrategyKey`
  ...
  - Required.
*/
- (instancetype)initWithVolumeSizeInMeters:(GLKVector3)volumeSize options:(NSDictionary*)options;

/** Update the current pose estimate from a depth frame and a CoreMotion gravity vector, using the strategy specified at
init.

- Parameter gravityInDeviceFrame: a gravity vector in IMU coordinates, e.g. as provided by CMDeviceMotion.gravity.
- Parameter depthFrame: the current processed depth from Structure Sensor. Can be `nil` if using the strategies
`STCameraPoseInitializerStrategyGravityAlignedAtOrigin` or
`STCameraPoseInitializerStrategyGravityAlignedAtVolumeCenter`.
- Parameter error: will contain detailed information if the estimation failed.

- Returns: TRUE on success, FALSE on failure.
*/
- (BOOL)updateCameraPoseWithGravity:(GLKVector3)gravityInDeviceFrame
                         depthFrame:(STDepthFrame*)depthFrame
                              error:(NSError* __autoreleasing*)error;

/** Determine which pixels of a depth frame are inside the current scanning volume.

- Parameter depthFrame: the current processed depth from Structure Sensor.
- Parameter outputMask: should point to an allocated buffer of (`depthFrame.width * depthFrame.height`) pixels. A mask
pixel will be 255 if the corresponding 3D point fits inside the volume, 0 if outside or if there is no depth.
*/
