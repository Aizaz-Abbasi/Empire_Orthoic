...
typedef NS_ENUM(NSInteger, STColorizerType) {
    ...
    */
    ...
    /// Generates a global texture map, and UV coordinates for each vertex of the mesh. Optimized for objects and
    /// people.
    STColorizerTextureMapForObject = 2,

    /// Generates a global texture map, and UV coordinates for each vertex of the mesh. Optimized for both objects and
    /// rooms
    STColorizerTextureMapGeneral = 3,
};
...
- Parameter mesh: The STMesh to colorize.
- Parameter scene: The STScene context.
- Parameter keyframes: Array of keyframes, e.g. as provided by STKeyFrameManager.
...
- `kSTColorizerPrioritizeFirstFrameColorKey`:
  ...
  - Defaults to `@NO`.
  - Particularly useful when we want to emphasize the appearance in the first frame, for example, when scanning a human
face.
